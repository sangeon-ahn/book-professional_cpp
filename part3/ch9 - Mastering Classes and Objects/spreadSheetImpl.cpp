...
// copy constructor
SpreadSheet::SpreadSheet(const SpreadSheet& src) : SpreadSheet{src.m_width, src.m_height, src.m_theApp} // 이 ctor-initialization을 통해 delegating constructor 호출되며 동적 메모리가 할당된다. 이후 {}에서 실제 셀데이터가 복사된다.
{
    for (size_t i { 0 }; i < m_width; ++i) {
        for (size_t j { 0 }; j < m_height; ++j) {
